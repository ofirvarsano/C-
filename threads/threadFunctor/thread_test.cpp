...
class IncGlobalInLoopFunc : public advcpp::Runnable {
public:
	IncGlobalInLoopFunc() {}
...
class IncGlobalInLoopNoMutexFunc : public advcpp::Runnable {
public:
	IncGlobalInLoopNoMutexFunc() {}
	virtual void Do() const
	{
		int i = 0;
		for ( ; i < LOOP_TIMES ; ++i) {
...
UNIT(inc_global_mutex)
	g = 0;
	IncGlobalInLoopFunc func1;
	try {
		advcpp::Thread t1(func1); 
		advcpp::Thread t2(func1);
...
UNIT(inc_global_without_mutex)
	g = 0;
	IncGlobalInLoopNoMutexFunc func1;
	try {
		advcpp::Thread t1(func1); 
		advcpp::Thread t2(func1);
